...
contract WrappedToken is SecureContract, ERC20
{
    ...
    uint public heldAmtReq_;

    constructor(address backingTokenContract, string memory name, string memory ticker, uint8 tokenDecimals, uint heldAmtReq, address fmtaAddress) SecureContract() ERC20(name, ticker)
    {
    ...
    function getUserTokenBalance (address user) internal view returns (uint) {
        return FMTA_.balanceOf(user);
    }
    
    function setHeldAmtReq (uint newAmt) public {
        require(hasRole(_ADMIN, msg.sender), "WrappedToken: setHeldAmtReq - Permission denied");
        heldAmtReq_ = newAmt;
    }
    ...
    {
        require(getUserTokenBalance(msg.sender) >= heldAmtReq_, "WrappedToken: Must hold required amount of FMTA");
    ...
    function transferBackingTokenFrom(address user, uint256 amount) public
    {
        ...
        require(getUserTokenBalance(user) >= heldAmtReq_, "WrappedToken: Must hold required amount of FMTA");
